...
├── cpt_code_existence_loader.py        # Parses AMA Category II/III PDFs into a local lookup table
├── requirements.txt
├── .gitignore
└── rcm_production_master.db            # Local SQLite store (gitignored — created at runtime)
```
...
The `feedback_memory` table and the Closed-Loop Feedback tab exist specifically to close this gap: as real 835 remittance outcomes are logged, they're blended into training data on each retrain cycle. In a production deployment, the synthetic baseline would be phased out once sufficient real-outcome volume accumulates.
